...
import { ShieldCheck, WalletCards, Layers, LogOut, Sparkles } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 border-b border-indigo-500/10 bg-slate-950/60 backdrop-blur-xl">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-xl font-bold tracking-tight hidden sm:inline-block bg-gradient-to-r from-violet-400 to-fuchsia-400 bg-clip-text text-transparent">CardVault</span>
          </Link>
            ...
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="relative h-10 w-10 rounded-full border-2 border-violet-500/30 hover:border-violet-500/60 p-0 transition-colors">
                ...
                <DropdownMenuContent align="end" className="w-56 bg-slate-900 border-indigo-500/20 text-slate-100 shadow-xl shadow-indigo-900/20">
                  <DropdownMenuLabel className="font-normal">
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-bold leading-none text-slate-200">{user.username}</p>
                      <p className="text-xs leading-none text-violet-400">Kolektor Lokal</p>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator className="bg-slate-800" />
                  <DropdownMenuItem onClick={logout} className="text-red-400 focus:text-red-300 focus:bg-red-950/50 cursor-pointer">
