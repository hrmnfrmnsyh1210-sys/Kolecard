  ...
  const displayItems = cards
    .map((card) => {
      ...
      const activeListing = listings.find(
        (l) => l.cardId === card.id && l.status === "Active",
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-violet-300/80 mt-1">
            Jelajahi seluruh kartu dari komunitas kolektor dan ajukan penawaran.
          </p>
        </div>
      ...
      {displayItems.length === 0 ? (
        <div className="text-center py-20 border border-dashed border-violet-500/20 rounded-xl bg-violet-950/10 backdrop-blur-sm">
          ...
          <h3 className="text-lg font-medium text-slate-300">
            Belum ada kartu di Galeri
          </h3>
          <p className="text-slate-500 mt-1 mb-6">
            Mulai tambahkan kartu ke koleksiku untuk membagikannya di Galeri.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {displayItems.map((item) => (
            <Card
              key={item.card.id}
              className="bg-slate-900/60 backdrop-blur border-slate-800/60 overflow-hidden hover:border-violet-500/50 transition-all duration-300 flex flex-col group hover:shadow-[0_0_20px_rgba(139,92,246,0.15)] relative"
            >
              {item.activeListing && (
              ...
              <div className="aspect-[3/4] relative bg-slate-950/50 flex items-center justify-center p-4">
                {item.card.imageUrl ? (
                  <img
                    ...
                    alt={item.card.name}
                    className={`max-w-full max-h-full object-contain drop-shadow-[0_10px_20px_rgba(0,0,0,0.5)] transition-transform duration-500 ${item.activeListing ? "opacity-90" : "group-hover:scale-105"}`}
                  />
                ) : (
              ...
              <CardContent className="p-5 flex-1 bg-gradient-to-b from-slate-900/0 to-slate-900/80">
                <div className="text-xs font-medium text-violet-400 mb-1.5">
                  {item.card.game} •{" "}
                  <span className="text-fuchsia-400">{item.card.rarity}</span>
                </div>
                <h3 className="font-bold text-slate-100 text-lg leading-tight mb-3 line-clamp-2">
                  {item.card.name}
                </h3>
              ...
              <CardFooter className="p-4 flex items-center justify-between gap-3 border-t border-slate-800/50 bg-slate-900">
                <Link
                  ...
                  className="text-xs text-slate-400 truncate flex items-center gap-1.5 hover:text-slate-200 transition-colors group/user"
                >
                  <div className="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-300 flex items-center justify-center border border-indigo-500/30 group-hover/user:bg-indigo-500/40 transition-colors">
                    {item.owner?.username?.substring(0, 1).toUpperCase() || "?"}
                  </div>
                  <span className="text-slate-300 group-hover/user:text-slate-100 transition-colors">
        ...
        <DialogContent className="bg-slate-900 border-indigo-500/30 text-slate-50 sm:max-w-md shadow-2xl shadow-indigo-900/20">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              Buat tawaran harga ke pemilik kartu. Pemilik bisa memilih untuk
              menerima atau menolak tawaranmu.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 border-y border-slate-800 my-2 flex items-center gap-4 bg-slate-950/30 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 mb-2">
                Pemilik: {selectedCard?.owner?.username}
              </p>
            </div>
          </div>
          <form onSubmit={handleMakeOffer} className="space-y-4 py-2">
            <div className="space-y-3">
              <Label htmlFor="offerPrice" className="text-slate-300">
                Harga Tawaran (Rp)
              </Label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 font-medium tracking-wider">
                  Rp
                </span>
                <Input
                  id="offerPrice"
            ...
            {offerPrice && !isNaN(parseInt(offerPrice)) && (
              <p className="text-sm text-fuchsia-400/80 bg-fuchsia-500/10 p-3 rounded-md border border-fuchsia-500/20 text-center font-medium">
                ...
                <span className="font-bold text-fuchsia-400">
                  {formatIDR(parseInt(offerPrice))}
                </span>{" "}
                akan dikirim ke {selectedCard?.owner?.username}.
              </p>
            )}
            <DialogFooter className="pt-2 gap-2 sm:gap-0">
              ...
              <Button
                type="submit"
                className="bg-fuchsia-600 hover:bg-fuchsia-500 text-white shadow-lg shadow-fuchsia-900/40 font-bold"
              >
                Ajukan Tawaran
              </Button>
            </DialogFooter>
